...
if(isset($_SESSION['user_id'])){
    ...
    if($role==1){
        ...
        $sql3 = "SELECT * FROM reservation WHERE user_fk = $user";
        $result3 = $conn->query($sql3);
        $brisanje=[];
        while($row = $result3->fetch_assoc()) {
            $brisanje[]=intval($row["reserv_id"]);
        }

        $sql2 = "SELECT * FROM sverezervacije WHERE id_user= $user";
        $result2 = $conn->query($sql2);
        $mojerez = [];
        while($row2 = $result2->fetch_assoc()) {
            $mojerez[]=$row2['id_rez'];
        }
        foreach ($mojerez as $rez){
            $rez = intval($rez);
            if (empty($brisanje)){

            }
            else{
                if (in_array($rez, $brisanje))
                {

                }
                else{
                    echo '<p style="font-size: 15px">Your reservation with id '.$rez.' has been deleted by admin</p>';
                }
            }
